 ...
 *
 * @see com.transferwise.sequencelayout.SequenceStepView
 */
class SequenceLayoutView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
    FrameLayout(context, attrs, defStyleAttr), SequenceStepView.OnStepChangedListener {
    ...
    /**
     * Removes all contained [com.transferwise.sequencelayout.SequenceStepView]s
     */
    fun removeAllSteps() {
    ...
    /**
     * Replaces all contained [com.transferwise.sequencelayout.SequenceStepView]s with those provided and bound by the adapter
     */
    fun <T> setAdapter(adapter: SequenceAdapter<T>) where T : Any {
        ...
        for (i in 0 until count) {
            ...
            val view = SequenceStepView(context)
    ...
    private fun placeDots() {
        ...
        stepsWrapper.children().forEachIndexed { i, view ->
            val sequenceStep = view as SequenceStepView
    ...
    private val animateToActive = {
        ...
        val activeStepIndex =
            stepsWrapper.children().indexOfFirst { it is SequenceStepView && it.isActive() }
